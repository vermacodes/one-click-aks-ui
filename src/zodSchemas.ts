...
export const labDescriptionSchema = z
  ...
  .max(
    15000,
    "Lab description too long. Wanna write more? Add wiki page and link here.",
  );
...
export const labMessageSchema = z
  ...
  .max(
    15000,
    "Lab message too long. Wanna write more? Add wiki page and link here.",
  );
...
// user principal ID
export const userPrincipalIdSchema = z
  .string()
  .uuid("User Principal ID must be a valid UUID.");
...
// user Alias
export const userAliasSchema = z
  .string()
  .regex(
    /^([vt]-[a-z]+|[a-z]+)$/,
    "User Alias must be 3-50 characters and contain lowercase letters. When using a dash, it must be at the second position and the first character must be 'v' or 't' (e.g., v-abc, t-xyz)",
  )
  .min(3, "User Alias must be at least 3 characters.")
  .max(50, "User Alias must not exceed 50 characters.");
