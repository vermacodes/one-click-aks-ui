...
export default function CreateNewDeployment({ variant, tooltipMessage, tooltipDelay, children }: Props) {
	const [showModal, setShowModal] = useState<boolean>(false);
	const [newWorkSpaceName, setNewWorkSpaceName] = useState<string>("");
	const [isNewWorkspaceNameModified, setIsNewWorkspaceNameModified] = useState<boolean>(false);
	const { actionStatus } = useContext(WebSocketContext);
	const [showPleaseWaitModal, setShowPleaseWaitModal] = useState<boolean>(false);
...
function Modal({
	...
	function handleWorkspaceNameTextField(event: React.ChangeEvent<HTMLInputElement>) {
				...
				<div className="flex flex-col gap-y-2 pt-4">
					<div className="flex gap-x-2">
						...
						<Button
							...
							disabled={!deploymentNameSchema.safeParse(newWorkSpaceName).success || actionStatus.inProgress}
						>
							Create
						</Button>
