...
export default function AutoDestroySwitch({ disabled, deployment, label }: AutoDestroySwitchProps) {
	...
	function handleAutoDeleteChange() {
		...
		if (deployment.deploymentAutoDelete) {
			deploymentAutoDeleteUnixTime = calculateNewEpochTimeForDeployment(deployment);
		}
