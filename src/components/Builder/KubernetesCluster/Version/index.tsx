...
export default function Version({ index }: Props) {
	...
	const currentVersion = lab?.template?.kubernetesClusters[index]?.kubernetesVersion;
	...
	const handleOnSelect = (patchVersion: PatchVersions) => {
		...
		if (newLab?.template && newLab.template.kubernetesClusters[index]) {
			newLab.template.kubernetesClusters[index].kubernetesVersion = patchVersionKey;
	...
	const renderItem = (patchVersion: PatchVersions) => {
		...
		const classes = key === currentVersion ? "bg-green-300 hover:text-slate-900 dark:text-slate-900" : "";
