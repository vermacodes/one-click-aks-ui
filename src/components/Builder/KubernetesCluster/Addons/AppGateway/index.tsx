...
export default function AppGateway({ index }: Props) {
	...
	if (lab && lab.template && lab?.template?.kubernetesClusters[index]?.networkPluginMode === "overlay") {
	...
	function handleOnChange() {
		...
		if (newLab?.template?.kubernetesClusters[index]?.addons?.appGateway === undefined) {
			return;
		}
	...
	const checked = lab.template.kubernetesClusters[index]?.addons?.appGateway ?? false;
	const disabled =
		lab.template.kubernetesClusters.length === 0 ||
		lab.template.virtualNetworks.length === 0 ||
		lab.template.kubernetesClusters[index].networkPluginMode === "overlay";
