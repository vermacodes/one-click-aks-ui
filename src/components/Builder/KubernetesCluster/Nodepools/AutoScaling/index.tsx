...
export default function AutoScaling({ index }: Props) {
	...
	const handleOnChange = () => {
		...
		if (cluster?.defaultNodePool?.enableAutoScaling !== undefined && newLab !== undefined) {
			cluster.defaultNodePool.enableAutoScaling = !cluster.defaultNodePool.enableAutoScaling;
	...
	const checked = lab?.template?.kubernetesClusters[index]?.defaultNodePool?.enableAutoScaling ?? false;
