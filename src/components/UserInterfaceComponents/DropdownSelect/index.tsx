...
export default function DropdownSelect<T>({
	...
	return (
		<div className={`${isMenuOpen ? "relative" : ""} inline-block w-full text-left`}>
			<Tooltip message={tooltipMessage} direction={tooltipDirection} delay={tooltipDelay}>
				<div
					className={`${
				...
				>
					<div>{heading}</div>
					<p>
						<FaChevronDown className={`${isMenuOpen && "-rotate-180"} transition duration-500`} />
					</p>
				</div>
			</Tooltip>
			{isMenuOpen && (
...
const DropdownMenu = <T,>({
	...
	return (
		<div
			className={`absolute right-0 z-10 mt-1 ${height} w-full origin-top-right items-center gap-y-2 overflow-y-auto  overflow-x-hidden rounded border border-slate-500 bg-slate-100 p-2 scrollbar-thin scrollbar-thumb-slate-400 dark:bg-slate-800 dark:scrollbar-thumb-slate-600`}
