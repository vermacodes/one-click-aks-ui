...
export default function ManagedServerRegistration({}: Props) {
  // Check if hostname includes 'dev'
  const isDevEnvironment =
    typeof window !== "undefined" && window.location.hostname.includes("dev");

  // Conditionally set the command based on environment
  const registrationCommand = isDevEnvironment
    ? "curl -o actlabs.sh -sLO https://aka.ms/ActlabsManagedServer; chmod +x actlabs.sh; ./actlabs.sh dev; rm actlabs.sh"
    : "curl -o actlabs.sh -sLO https://aka.ms/ActlabsManagedServer; chmod +x actlabs.sh; ./actlabs.sh; rm actlabs.sh";
        ...
        <ol className="ml-6 list-decimal">
          <li className="py-1">
            ...
            <p className="mt-2 text-sm text-slate-500">
              You can also run these steps on your system if you use linux (WSL)
              and have latest version of Azure CLI installed. We recommend using
              Azure Cloud Shell.
            </p>
          </li>
          <li className="py-1">
