...
export default function StartCommand({}: Props) {
  const bashCommand = "curl -o actlabs.sh -sLO https://aka.ms/ACTLabStart; chmod +x actlabs.sh; ./actlabs.sh; rm actlabs.sh";
    ...
    >
      <div className="flex items-center justify-between gap-4 py-2 px-4 text-xl">
        <div className="flex justify-start gap-4 text-xl">
          <button
            className={`${language === "bash" ? "border-sky-500 text-sky-500" : "border-transparent"} " border-slate-500" border-b-2`}
            onClick={() => {
              setLanguage("bash");
          ...
          <button
            className={`${language === "powershell" ? "border-sky-500 text-sky-500" : "border-transparent"} " border-slate-500" border-b-2`}
            onClick={() => {
