...
export default function SaveLabModal({ lab, showModal, setShowModal }: Props) {
  ...
  const { mutateAsync: createLabWithSupportingDocument } =
    useCreateLabWithSupportingDocument();
  ...
  return (
    <div
      className="bg-opacity-80 dark:bg-opacity-80 fixed inset-0 z-20 flex max-h-full max-w-full justify-center bg-slate-800 dark:bg-slate-100"
      onClick={(e) => {
        e.stopPropagation();
      }}
    >
      <div
        className={`${defaultScrollbarStyle} my-10 h-[90%] w-2/3 space-y-4 overflow-x-hidden overflow-y-auto rounded-sm bg-slate-100 p-5 dark:divide-slate-700 dark:bg-slate-900`}
        onClick={(e) => {
          e.stopPropagation();
        }}
      >
        <div className="mb-5 flex w-100 justify-between border-b-2 border-b-slate-500 pb-2">
