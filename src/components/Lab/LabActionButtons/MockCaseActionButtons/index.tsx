...
export default function MockCaseActionButtons({ lab }: Props) {
	...
	useEffect(() => {
		...
		if (lab.type === "mockcase" && lab.supportingDocumentId !== "") {
			const downloadSupportingDocumentButton: ButtonContainerObj = {
				id: "downloadSupportingDocumentButton",
				order: 2,
				button: (
					<DownloadButton key={"downloadSupportingDocumentButton"} lab={lab} variant="secondary-text">
						<span>
							<FaDownload />
						</span>
						Documents
			...
			upsertButton(downloadSupportingDocumentButton);
		} else {
			deleteButton("downloadSupportingDocumentButton");
		}
	}, [profile, lab]);
