...
export default function ButtonContainer({ buttons, setButtons, overflowButtons, setOverflowButtons }: Props) {
	...
	useEffect(() => {
		...
		const resizeObserver = new ResizeObserver(() => {
			...
			if (containerWidth && buttonWidth) {
				...
				if (Object.keys(sortedButtons).length < maxButtons && Object.keys(overflowButtons).length > 0) {
					setButtons({ ...sortedButtons });
					setOverflowButtons({});
				}
				if (Object.keys(sortedButtons).length > maxButtons && Object.keys(buttons).length !== maxButtons) {
					setButtons(Object.fromEntries(Object.entries(sortedButtons).slice(0, maxButtons)));

					setOverflowButtons(Object.fromEntries(Object.entries(sortedButtons).slice(maxButtons)));
				}
			}
		});
	...
	return (
		<div className="flex w-full" ref={containerRef}>
			<div className="flex w-full justify-between">
				<div className="flex w-full gap-x-4">{Object.values(buttons).map((button) => button.button)}</div>
