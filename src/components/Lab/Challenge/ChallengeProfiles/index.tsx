  ...
  return (
    ...
    >
      <div className="flex flex-row flex-wrap justify-between gap-2">
        ...
        {/* Show "Challenge Others" button if:
            - Current user is a lab owner (always allowed), OR
            - Challenge forwarding is enabled AND the current user is a challenger
              who has completed the challenge and has forwarded fewer than 2 challenges */}
        {(() => {
          //if (meOwner) return true;
          if (!meChallenger || !challenges || !myProfile) return false;
          if (!lab.labControls.challengeLabAllowChallengeForwarding)
            return false;

          const hasCompleted = challenges.some(
            (c) =>
              c.userId === myProfile.userPrincipal &&
              c.labId === lab.id &&
              c.status === "completed",
          );
          const forwardCount = challenges.filter(
            (c) =>
              c.createdBy === myProfile.userPrincipal && c.labId === lab.id,
          ).length;

          return hasCompleted && forwardCount < 2;
        })() && (
