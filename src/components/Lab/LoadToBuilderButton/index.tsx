...
export default function LoadToBuilderButton({ variant, children, lab }: Props) {
	...
	const hasAccess = (lab: Lab | undefined, profile: Profile | undefined) => {
		...
		const { category, rbacEnforcedProtectedLab, owners, editors, viewers } = lab;
	...
	return (
		<Button variant={variant} onClick={onClickHandler} disabled={hasAccess(lab, profile) || lab === undefined}>
			<span>
				<FaTools />
			</span>
			{children}
