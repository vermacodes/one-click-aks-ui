...
export default function SaveLabPage() {
  ...
  const { mutateAsync: upsertSupportingDocument } =
    useUpsertSupportingDocument();
  ...
  async function onConfirmCreateLab() {
    ...
    try {
      ...
      if (supportingDocument) {
        ...
        const upsertResponse = await toast.promise(
          upsertSupportingDocument(supportingDocument),
          {
  ...
  return (
    <PageLayout
      heading={`${lab.id === "" ? "Save New Lab" : "Save Lab - " + lab.name}`}
    >
      <div
        className={`${defaultScrollbarStyle} mb-4 space-y-4 overflow-x-hidden overflow-y-auto rounded-sm bg-slate-100 p-5 dark:divide-slate-700 dark:bg-slate-900`}
      >
